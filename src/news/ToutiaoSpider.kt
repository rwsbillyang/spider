    ...
    override fun extract(url: String, map: MutableMap<String, String?>) {
        ...
        if (newUrl.contains("/article/")) {
            ...
            webDriver.findElements(By.cssSelector("div.tt-video-box"))?.forEach {
                ...
                val v =
                    "<p><video src=\"$src\" controls loop autoPlay=\"false\" preload=\"metadata\" width=\"100%\"  playsInline webkit-playsinline x5-playsinline x6-playsinline x5-video-player-fullscreen=\"false\"  x5-video-player-type=\"h5\" x-webkit-airplay=\"allow\"></p>"
                //log.info("c=$c, v=$v")
            ...
            map[Spider.USER] =
                webDriver.findElement(By.cssSelector("div.author-info>div.author-info-name")).text ?: "头条"
            ...
            map[Spider.VIDEO_COVER] =
                webDriver.findElements(By.tagName("xg-poster"))?.firstOrNull()?.getAttribute("style")
                    ?.substringAfter("url(\"")?.removeSuffix("\");")
            ...
            val imgs = webDriver.findElement(By.cssSelector("div.image-list"))
                .findElements(By.tagName("div")).firstOrNull()?.getAttribute("outerHTML")
